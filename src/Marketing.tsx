...
function Header({setView}:{setView:(v:AppView)=>void}){
  ...
  return <header className="marketing-header"><div className="marketing-nav wrap">
    ...
    <nav className={open?'open':''}><button onClick={()=>jump('autofahrer')}>Für Autofahrer</button><button onClick={()=>jump('werkstaetten')}>Für Werkstätten</button><button onClick={()=>{setOpen(false);setView('finder')}}>Werkstatt finden</button><button onClick={()=>jump('sicherheit')}>Sicherheit</button></nav>
