...
export function Marketing({setView}:{setView:(v:AppView)=>void}){
 return <><main className="marketing">
  <section className="hero"><div className="ambient a"/><div className="ambient b"/><div className="wrap hero-grid"><div className="hero-copy"><div className="eyebrow"><span>NEU</span> MOTORATLAS VERBINDET AUTOFAHRER UND WERKSTATT</div><h1>Schneller zur Hilfe. <em>Weniger Aufwand auf beiden Seiten.</em></h1><p><strong>Für Autofahrer:</strong> schneller Termin anfragen, Kosten besser kontrollieren und jederzeit den aktuellen Stand sehen. <strong>Für Werkstätten:</strong> Personal entlasten, Abläufe beschleunigen, professioneller auftreten und mehr Umsatzpotenzial nutzen.</p><div className="hero-actions"><button className="btn primary xl" onClick={()=>setView('finder')}><Search size={18}/> Werkstatt finden</button><button className="btn glass xl" onClick={()=>setView('workshop-info')}>Für Werkstätten <ArrowRight size={18}/></button></div><div className="trust-row"><TrustPill>Schnelle Terminabstimmung</TrustPill><TrustPill>Fahrzeugspezifische Aufträge</TrustPill><TrustPill>Status in Echtzeit</TrustPill></div></div><ProductWindow setView={setView}/></div></section>
  ...
  <section className="section wrap directory" id="finden"><div><SectionIntro align="left" kicker="DEUTSCHLANDWEIT" title={<>Werkstatt finden. <em>Beziehung aufbauen.</em></>} text="Verifizierte Werkstätten zeigen Leistungen, Öffnungszeiten und ob sie neue Kunden aufnehmen."/><div className="directory-points"><span><CheckCircle2/> Werkstatt stellt ihr Profil selbst professionell dar</span><span><CheckCircle2/> Kunde fragt die Aufnahme an</span><span><CheckCircle2/> Erst nach Freigabe entstehen Aufträge</span><span><CheckCircle2/> Stammwerkstatt lässt sich jederzeit wechseln</span></div><button className="btn primary xl" onClick={()=>setView('finder')}>Werkstätten durchsuchen <ArrowRight size={17}/></button></div><GermanyMap/></section>
...
function Footer({setView}:{setView:(v:AppView)=>void}){return <footer className="footer"><div className="wrap footer-grid"><div><Brand/><p>Die digitale Verbindung zwischen Autofahrer und Werkstatt.</p></div><div><b>Produkt</b><button onClick={()=>setView('customer-info')}>Für Autofahrer</button><button onClick={()=>setView('workshop-info')}>Für Werkstätten</button><button onClick={()=>setView('finder')}>Werkstatt finden</button></div><div><b>Zugang</b><button onClick={()=>setView('login')}>Anmelden</button><button onClick={()=>setView('office')}>Produktdemo</button></div><div><b>Plattform</b><button onClick={()=>setView('security-info')}>Sicherheit</button><span>Deutschlandweit</span><span>Web · PWA · Android</span></div></div><div className="wrap footer-bottom"><span>© 2026 MotorAtlas</span><span className="footer-legal"><button onClick={()=>setView('privacy')}>Datenschutz</button><span>·</span><button onClick={()=>setView('imprint')}>Impressum</button></span></div></footer>}
